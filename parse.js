...
// run through the tokens and form strings
function strrun () {
	var outstr='';
	let count = id2tok.length;
	for (let i = 0; i < count; i++) {
		 if (id2tok[i]) outstr+=stringify(id2tok[i], true)+";\n";
	}
	return outstr;
}
...
// convert a group of tokens into an HTML string
function stringify (tokens, HTML=false) {
	let outstr='';
	let pc = 0;
